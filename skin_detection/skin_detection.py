# -*- coding: utf-8 -*-
"""
Created on Tue Jul 26 19:50:27 2016
...
clf = svm.SVC(kernel='linear', random_state=2016, C=0.01)
...
print("  train: P={0:f}, R={1:f}".format(precision_score(labels,labels_pred), recall_score(labels,labels_pred)))

features_test, labels_test = fetch_data(img_lbl_filenames_test)
labels_pred = clf.predict(features_test)
print("  test : P={0:f}, R={1:f}".format(precision_score(labels_test,labels_pred), recall_score(labels_test,labels_pred)))
...
for f in img_lbl_filenames_train:
    ...
    print("  training image {0:s}:  P={1:f}, R={2:f}".format(f_img, precision_score(y_true, y_pred), recall_score(y_true,y_pred)))
print('done in %.2fs.' % (time() - t0))
